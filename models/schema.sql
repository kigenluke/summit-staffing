...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bookings' AND column_name = 'source_shift_id') THEN
...
CREATE INDEX IF NOT EXISTS bookings_source_shift_id_idx ON bookings (source_shift_id) WHERE source_shift_id IS NOT NULL;
