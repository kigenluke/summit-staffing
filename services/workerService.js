...
export const getMe = async () => {
  return safeRequest('GET', '/api/workers/me');
};
...
export const getWorkers = async (filters = {}) => {
  ...
  return safeRequest('GET', `/api/workers${query.toString() ? `?${query.toString()}` : ''}`);
};
...
export const searchWorkers = async (latOrParams, lng, radius, skills) => {
  ...
  return safeRequest('GET', `/api/workers/search${query.toString() ? `?${query.toString()}` : ''}`);
};

export const getWorkerById = async (id) => {
  return safeRequest('GET', `/api/workers/${id}`);
};

export const updateWorker = async (id, data) => {
  return safeRequest('PUT', `/api/workers/${id}`, data);
};
...
export const uploadProfilePhoto = async (workerId, file, onUploadProgress) => {
  ...
  const { data, error } = await api.post(`/api/workers/${workerId}/profile-photo`, form, {
...
export const uploadDocument = async (
  ...
  const { data, error } = await api.post(`/api/workers/${workerId}/documents`, form, {
    headers: { 'Content-Type': 'multipart/form-data' },
    onUploadProgress,
  });
  return {
...
export const uploadDocumentsBulk = async (
  ...
  const { data, error } = await api.post(`/api/workers/${workerId}/documents/bulk`, form, {
    headers: { 'Content-Type': 'multipart/form-data' },
    onUploadProgress,
  });
  return {
...
export const addSkill = async (workerId, skill) => {
  return safeRequest('POST', `/api/workers/${workerId}/skills`, { skill_name: skill });
};

export const removeSkill = async (workerId, skillId) => {
  return safeRequest('DELETE', `/api/workers/${workerId}/skills/${skillId}`);
};

export const updateAvailability = async (workerId, schedule) => {
  return safeRequest('PUT', `/api/workers/${workerId}/availability`, { availability: schedule });
};
