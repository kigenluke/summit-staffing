...
export function BudgetScreen() {
  ...
  const saveTarget = async () => {
    ...
    if (Number.isNaN(value) || value < 0) {
      Alert.alert('Invalid target', 'Please enter a valid monthly budget amount.');
      return;
    }
    setSavingTarget(true);
    try {
      const { error } = await api.put(`/api/participants/${profile.id}`, { monthly_budget: value });
      if (error) {
        Alert.alert('Error', error.message || 'Failed to save target');
      } else {
        Alert.alert('Saved', 'Monthly budget target updated.');
        await load();
      }
    } catch (_) {
      ...
      <Card style={{ marginBottom: Spacing.md }}>
        ...
        <View style={{ marginTop: Spacing.md, borderTopWidth: 1, borderTopColor: Colors.border, paddingTop: Spacing.md }}>
          ...
          <TextInput
            ...
            placeholder="Enter monthly budget target"
