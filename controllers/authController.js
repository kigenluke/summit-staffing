...
const deleteAccount = async (req, res) => {
  try {
    ...
    try {
      ...
      const userRes = await client.query('SELECT id, role FROM users WHERE id = $1 LIMIT 1', [userId]);
      if (userRes.rowCount === 0) {
        await client.query('ROLLBACK');
      ...
      // Bookings reference workers/participants with ON DELETE RESTRICT,
      // so we remove related bookings first and let cascades clean child tables.
      if (role === 'worker') {
        const workerRes = await client.query('SELECT id FROM workers WHERE user_id = $1 LIMIT 1', [userId]);
        if (workerRes.rowCount > 0) {
          const workerId = workerRes.rows[0].id;
          await client.query('DELETE FROM bookings WHERE worker_id = $1', [workerId]);
        }
      } else if (role === 'participant') {
        const participantRes = await client.query('SELECT id FROM participants WHERE user_id = $1 LIMIT 1', [userId]);
        if (participantRes.rowCount > 0) {
          const participantId = participantRes.rows[0].id;
          await client.query('DELETE FROM bookings WHERE participant_id = $1', [participantId]);
        }
      }
